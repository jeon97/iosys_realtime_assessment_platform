...
응시자 API는 답안뿐 아니라 접속, 카메라, 채팅과 이상행위 이벤트도 처리하였습니다. 이 데이터는 화면에 즉시 보여야 하는 현재 상태와 사후 확인을 위한 이력이 모두 필요합니다.
...
- 이벤트 ID: 공개 예제에서 추가한 재전송 중복 처리 차단

카메라 이벤트는 촬영 영상을 저장하는 기능이 아니라 상태 변경을 전달하는 흐름으로 정리하였습니다. 채팅 내용과 개인정보도 공개 예제에 넣지 않았습니다.
